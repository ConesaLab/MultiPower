...
# New function MultiGroupPower for multiple comparisons (more than two groups)
...
MultiGroupPower = function(data, groups, type, comparisons = NULL,
                           d0 = 0.8, p1 = 0.2, omicPower = 0.6, averagePower = 0.85, dispPerc = 75,
                           fdr = 0.05, alpha = 0.05,cost = 1, equalSize = TRUE, max.size = 200, omicCol = NULL,
  ...
  if (is.null(dim(p1))) {
    if (length(p1) == 1) {
      p1matrix = matrix(p1, nrow = length(data), ncol = ncol(comparisons))
    } else {
      if (length(p1) != length(data)) stop("Please, provide a p1 value for each omic or a unique value equal for all of them.")
      p1matrix = matrix(NA, nrow = length(data), ncol = ncol(comparisons))
      for (i in 1:ncol(p1matrix)) p1matrix[,i] = p1
    }
  } else {
    p1matrix = p1
  }
  ...
  for (i in 1:length(output)) {
    ...
    output[[i]] = MultiPower(data = data2, groups = groups2, type, d0, p1 = p1matrix[,i], omicPower, averagePower,
                             dispPerc, fdr, alpha, cost, equalSize, max.size, omicCol, powerPlots)
  }

  output$GlobalSummary = output[[1]]$summary

  output$GlobalSummary[,"DEperc"] = apply(sapply(output[-length(output)], function(x) x$summary[,"DEperc"]), 1,
                                          function (y) {
                                            if (length(unique(y)) == 1) return(unique(y))
                                            if (length(unique(y)) != 1) return(paste(range(y, na.rm = T), collapse = "-"))})

  output$GlobalSummary[,"delta"] = apply(sapply(output[-length(output)], function(x) x$summary[,"delta"]), 1,
                                         function (y) {
                                           if (length(unique(y)) == 1) return(unique(y))
                                           if (length(unique(y)) != 1) return(paste(range(y, na.rm = T), collapse = "-"))})

  output$GlobalSummary[,"minFC"] = apply(sapply(output[-length(output)], function(x) x$summary[,"minFC"]), 1,
                                         function (y) {
                                           if (length(unique(y)) == 1) return(unique(y))
                                           if (length(unique(y)) != 1) return(paste(range(y, na.rm = T), collapse = "-"))})

  output$GlobalSummary[,"meanCounts"] = apply(sapply(output[-length(output)], function(x) x$summary[,"meanCounts"]), 1,
                                         function (y) {
                                           if (length(unique(y)) == 1) return(unique(y))
                                           if (length(unique(y)) != 1) return(paste(range(y, na.rm = T), collapse = "-"))})

  output$GlobalSummary[,"dispersion"] = apply(sapply(output[-length(output)], function(x) x$summary[,"dispersion"]), 1,
                                         function (y) {
                                           if (length(unique(y)) == 1) return(unique(y))
                                           if (length(unique(y)) != 1) return(paste(range(y, na.rm = T), collapse = "-"))})
